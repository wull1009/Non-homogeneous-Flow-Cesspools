...
# 定义含泥量
p = [0.04, 0.05, 0.06, 0.05]*180
q = [0.96, 0.95, 0.94, 0.95]*180
V = []
out = []
f = []
s = []
...
for sed_hour in range(12, 25):
    ...
    for i in range(30):
        for j in range(24):
            c.append(((24-sed_hour)/24*p[i*24+j-sed_hour]+q[i*24+j-sed_hour])*raw_data2[j])
    # 建模
    ...
    for i in range(720):
        if i < sed_hour - 1:
            ...
            while c[i - sed_hour] > 0:
                if i - index >= 24:
                    if q[i - sed_hour] * a[index] > c[i - sed_hour]:
                        a[index] = (q[i - sed_hour] * a[index] - c[i - sed_hour])/q[i - sed_hour]
                        sed = sed + p[i-sed_hour]*c[i - sed_hour]
                        c[i - sed_hour] = 0
                    else:
                        c[i - sed_hour] = c[i - sed_hour] - q[i-sed_hour] * a[index]
                        sed = sed + p[i-sed_hour] * a[index]
                        a[index] = 0
                        index = index + 1
                else:
                    pp = ((i - index) / 24) * p[i-sed_hour]
                    qq = 1 - pp
